...
- (void)setupLabels {
	...
	label.textColor = [UIColor colorWithRed:0.024 green:0.243 blue:0.271 alpha:1];
    ...
    if ([isMiniSize intValue]==1) {
        
        boxRect = CGRectMake(roundf((allRect.size.width-30) / 2),roundf((allRect.size.height-30) / 2), 30, 30);
        imgview.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleTopMargin|UIViewAutoresizingFlexibleHeight|UIViewAutoresizingFlexibleLeftMargin|UIViewAutoresizingFlexibleRightMargin|UIViewAutoresizingFlexibleBottomMargin;
    }else{
        
        if (self.description.length>0||label.text.length>0) {
            boxRect = CGRectMake(roundf((allRect.size.width - 40) / 2) + xOffset, roundf((allRect.size.height - 40) / 2) + yOffset-30, 40, 40);
        }else{
            
            boxRect = CGRectMake((allRect.size.width - 40) / 2, (allRect.size.height - 40) / 2, 40, 40);
        }
