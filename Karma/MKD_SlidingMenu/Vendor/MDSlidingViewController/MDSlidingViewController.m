...
NSString *const MDSlidingViewUnderRightWillAppear   =@"MDSlidingViewUnderRightWillAppear";
NSString *const MDSlidingViewUnderLeftWillAppear    =@"MDSlidingViewUnderLeftWillAppear";
NSString *const MDSlidingViewUnderLeftWillDisappear = @"MDSlidingViewUnderLeftWillDisappear";
NSString *const MDSlidingViewUnderRightWillDisappear = @"MDSlidingViewUnderRightWillDisappear";
NSString *const MDSlidingViewTopDidAnchorLeft        = @"MDSlidingViewTopDidAnchorLeft";
NSString *const MDSlidingViewTopDidAnchorRight       = @"MDSlidingViewTopDidAnchorRight";
NSString *const MDSlidingViewTopWillReset            = @"MDSlidingViewTopWillReset";
NSString *const MDSlidingViewTopDidReset             = @"MDSlidingViewTopDidReset";

@interface MDSlidingViewController()
...
- (MDSlidingViewController *)slidingViewController
{
  UIViewController *viewController = self.parentViewController;
  while (!(viewController == nil || [viewController isKindOfClass:[MDSlidingViewController class]]))
  {
    viewController = viewController.parentViewController;
      
    //   return (MDSlidingViewController *)viewController;
  }
...
@implementation MDSlidingViewController
...
- (void)setUnderLeftWidthLayout:(MDViewWidthLayout)underLeftWidthLayout
{
  if (underLeftWidthLayout == MDVariableRevealWidth && self.anchorRightPeekAmount <= 0) {
    [NSException raise:@"Invalid Width Layout" format:@"anchorRightPeekAmount must be set"];
  } else if (underLeftWidthLayout == MDFixedRevealWidth && self.anchorRightRevealAmount <= 0) {
...
- (void)setUnderRightWidthLayout:(MDViewWidthLayout)underRightWidthLayout
{
  if (underRightWidthLayout == MDVariableRevealWidth && self.anchorLeftPeekAmount <= 0) {
    [NSException raise:@"Invalid Width Layout" format:@"anchorLeftPeekAmount must be set"];
  } else if (underRightWidthLayout == MDFixedRevealWidth && self.anchorLeftRevealAmount <= 0) {
...
- (void)viewDidLoad
{
  ...
  self.resetStrategy = MDTapping | MDPanning;
...
- (void)setResetStrategy:(MDResetStrategy)theResetStrategy
{
  _resetStrategy = theResetStrategy;
  if (_resetStrategy & MDTapping) {
...
- (void)updateTopViewHorizontalCenterWithRecognizer:(UIPanGestureRecognizer *)recognizer
{
  ...
  if (recognizer.state == UIGestureRecognizerStateBegan) {
    ...
    if ([self underLeftShowing] && currentVelocityX > 100) {
      [self anchorTopViewTo:MDRight];
    } else if ([self underRightShowing] && currentVelocityX < 100) {
      [self anchorTopViewTo:MDLeft];
    } else {
      [self resetTopView];
    }
  }
...
- (void)anchorTopViewTo:(MDSide)side
{
  [self anchorTopViewTo:side animations:nil onComplete:nil];
}

- (void)anchorTopViewTo:(MDSide)side animations:(void (^)())animations onComplete:(void (^)())complete
{
  CGFloat newCenter = self.topView.center.x;
  
  if (side == MDLeft) {
    newCenter = self.anchorLeftTopViewCenter;
  } else if (side == MDRight) {
  ...
  [UIView animateWithDuration:0.25f animations:^{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      NSString *key = (side == MDLeft) ? MDSlidingViewTopDidAnchorLeft : MDSlidingViewTopDidAnchorRight;
      [[NSNotificationCenter defaultCenter] postNotificationName:key object:self userInfo:nil];
    });
  }];
}

- (void)anchorTopViewOffScreenTo:(MDSide)side
{
  [self anchorTopViewOffScreenTo:side animations:nil onComplete:nil];
}

- (void)anchorTopViewOffScreenTo:(MDSide)side animations:(void(^)())animations onComplete:(void(^)())complete
{
  CGFloat newCenter = self.topView.center.x;
...
{
  dispatch_async(dispatch_get_main_queue(), ^{
    [[NSNotificationCenter defaultCenter] postNotificationName:MDSlidingViewTopWillReset object:self userInfo:nil];
  });
...
- (void)topViewHorizontalCenterWillChange:(CGFloat)newHorizontalCenter
{
  CGPoint center = self.topView.center;
	...
	if (center.x >= self.resettedCenter && newHorizontalCenter == self.resettedCenter) {
		dispatch_async(dispatch_get_main_queue(), ^{
			[[NSNotificationCenter defaultCenter] postNotificationName:MDSlidingViewUnderLeftWillDisappear object:self userInfo:nil];
		});
	}
	
	if (center.x <= self.resettedCenter && newHorizontalCenter == self.resettedCenter) {
		dispatch_async(dispatch_get_main_queue(), ^{
			[[NSNotificationCenter defaultCenter] postNotificationName:MDSlidingViewUnderRightWillDisappear object:self userInfo:nil];
		});
	}
...
- (void)addTopViewSnapshot
{
  if (!self.topViewSnapshot.superview && !self.shouldAllowUserInteractionsWhenAnchored) {
    ...
    if (self.shouldAddPanGestureRecognizerToTopViewSnapshot && (_resetStrategy & MDPanning)) {
...
{
  dispatch_async(dispatch_get_main_queue(), ^{
    [[NSNotificationCenter defaultCenter] postNotificationName:MDSlidingViewUnderLeftWillAppear object:self userInfo:nil];
  });
...
{
  dispatch_async(dispatch_get_main_queue(), ^{
    [[NSNotificationCenter defaultCenter] postNotificationName:MDSlidingViewUnderRightWillAppear object:self userInfo:nil];
  });
...
{
  dispatch_async(dispatch_get_main_queue(), ^{
    [[NSNotificationCenter defaultCenter] postNotificationName:MDSlidingViewTopDidReset object:self userInfo:nil];
  });
...
- (void)updateUnderLeftLayout
{
  if (self.underLeftWidthLayout == MDFullWidth) {
    [self.underLeftView setAutoresizingMask:self.autoResizeToFillScreen];
    [self.underLeftView setFrame:self.view.bounds];
  } else if (self.underLeftWidthLayout == MDVariableRevealWidth && !self.topViewIsOffScreen) {
    CGRect frame = self.view.bounds;
    
    frame.size.width = frame.size.width - self.anchorRightPeekAmount;
    self.underLeftView.frame = frame;
  } else if (self.underLeftWidthLayout == MDFixedRevealWidth) {
    CGRect frame = self.view.bounds;
    
    frame.size.width = self.anchorRightRevealAmount;
    self.underLeftView.frame = frame;
  } else {
    [NSException raise:@"Invalid Width Layout" format:@"underLeftWidthLayout must be a valid MDViewWidthLayout"];
  }
}

- (void)updateUnderRightLayout
{
  if (self.underRightWidthLayout == MDFullWidth) {
  ...
  } else if (self.underRightWidthLayout == MDVariableRevealWidth) {
    CGRect frame = self.view.bounds;
  ...
  } else if (self.underRightWidthLayout == MDFixedRevealWidth) {
    CGRect frame = self.view.bounds;
    ...
    [NSException raise:@"Invalid Width Layout" format:@"underRightWidthLayout must be a valid MDViewWidthLayout"];
  }
}
